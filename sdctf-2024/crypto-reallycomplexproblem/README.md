...
\text{ (mod N)}$ and $|x_0| < N^{\frac{1}{n}}$, we can find $x_0$. >
...
- $g(x_0) \equiv 0 \text{ (mod b^k)}$ for some $b, k$
- $|x_0| \leq R$ Where R is the upper bound we discussed earlier
- The length of the coefficient vector of $g(R \cdot x)$ is small. (The coefficient vector refers to the vector containing the
    ...
    - Small is once again defined as being less than some bound based on $b, k$ and the degree of $g(x)$. However, it's
        not relevant to us because we will fulfill it at the end.

then $g(x_0) = 0$ over the integers too. That is, $x_0$ is an integer root. 

Great! let's use this on $f(x)$. Well... we can't use it just yet because the coefficients of the polynomial $f(R\cdotx) are
...
The script can be found in [`solve.sage`](./solve.sage)
